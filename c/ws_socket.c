...
lean_obj_res ws_listen(int port) {
  ...
  if (bind(fd,(struct sockaddr*)&addr,sizeof(addr))<0) { int e=errno; close(fd); return mk_io_error_from_errno(e); }
  if (listen(fd,16)<0) { int e=errno; close(fd); return mk_io_error_from_errno(e); }
  return lean_io_result_mk_ok(lean_box(fd));
}

lean_obj_res ws_accept(int listen_fd) {
  int cfd = accept(listen_fd, NULL, NULL);
  if (cfd < 0) return mk_io_error_from_errno(errno);
  return lean_io_result_mk_ok(lean_box(cfd));
}

lean_obj_res ws_close(int fd) {
  if (close(fd) < 0) return mk_io_error_from_errno(errno);
...
lean_obj_res ws_send_bytes(int fd, lean_object* ba) {
  ...
  if (sent < 0) return mk_io_error_from_errno(errno);
